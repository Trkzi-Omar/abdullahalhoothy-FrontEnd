...
const LandingFeatures = ({ t: copy }: LandingFeaturesProps) => {
  return (
    <section id="features" className="py-24 relative bg-brand-surface/50 border-y border-white/5">
      <div className="container mx-auto px-6">
        <div className="flex flex-col items-center mb-16 text-center">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">{copy.features.sub}</h2>
        </div>
          ...
          <div className="grid lg:grid-cols-2">
            <div className="p-8 lg:p-12 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 text-brand-purple font-bold mb-4">
                ...
                <span className="uppercase tracking-widest text-xs">{copy.features.visualBadge}</span>
              </div>
              <h3 className="text-2xl lg:text-3xl font-bold text-white mb-4">
                {copy.features.visualLabel}
              </h3>
              ...
              <div className="w-full aspect-[4/3] relative rounded-xl overflow-hidden shadow-2xl border border-white/10 bg-brand-card flex items-center justify-center group-hover:scale-[1.02] transition-transform duration-500">
                <img
                  ...
                  onError={(e) => {
                    ...
                    if (parent) {
                      parent.innerHTML =
                        `<div class="text-center p-4"><span class="block text-brand-purple mb-2 text-4xl">📄</span><span class="text-white font-bold">${t("image-missing")}</span><br/><span class="text-xs text-slate-400">${t("please-add-report-png")}</span></div>`;
                    }
                  }}
                />
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {copy.features.list.map((feat, idx) => {
