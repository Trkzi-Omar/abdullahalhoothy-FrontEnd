...
import './PharmacyReportForm.css';
import { FaArrowLeft, FaExclamationTriangle } from 'react-icons/fa';
import { PharmacyReportData, FormErrors, MetricKey } from '../../types/allTypesAndInterfaces';
import { TOTAL_STEPS, INITIAL_FORM_DATA } from './constants';
...
const PharmacyReportForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Pharmacy report submitted successfully:', res);
      setShowSuccessMessage(true);
      setCompletedSteps(Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1));

      // Check if we have a report URL to redirect to
      const reportUrl = res?.data?.metadata?.html_file_path || res?.report_url;
      if (reportUrl) {
      ...
      console.error('Error submitting pharmacy report:', error);
          ...
          <div className="bg-gem-gradient px-4 py-3 text-white">
            <div className="flex items-center justify-between">
              ...
              <div className="text-center flex-1">
                <h1 className="text-lg font-bold">Pharmacy Location Report</h1>
              </div>
